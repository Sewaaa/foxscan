...
// ── Token di colore adattativi light/dark ──────────────────────────────────────
// Usati come classi composte per evitare ripetizioni

// Card container
const card    = "rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.03]";
// Testo principale (titoli, numeri grandi)
const txtPri  = "text-slate-900 dark:text-slate-100";
// Testo secondario (sottotitoli, label sezione)
const txtSec  = "text-slate-700 dark:text-slate-200";
// Testo body (link, contenuto)
const txtBody = "text-slate-600 dark:text-slate-400";
// Testo muted (label uppercase, sottotesti)
const txtMut  = "text-slate-500 dark:text-slate-600";
// Testo dimmed (valori secondari, "saltati", footer)
const txtDim  = "text-slate-400 dark:text-slate-700";
// Divider
const divider = "divide-slate-100 dark:divide-white/[0.04]";
const border  = "border-slate-100 dark:border-white/[0.04]";
// Sfondo subtile
const bgSub   = "bg-slate-50 dark:bg-white/[0.03]";
// Sfondo hover
const bgHov   = "hover:bg-slate-50 dark:hover:bg-white/[0.03]";
// Track barre
const track   = "bg-slate-200 dark:bg-white/5";
// Skeleton
const skel    = "animate-pulse rounded-lg bg-slate-200 dark:bg-white/5";

// ── Icons ─────────────────────────────────────────────────────────────────────

const IconPlay      = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" /></svg>;
const IconRefresh   = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" /></svg>;
const IconClock     = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>;
const IconTrash     = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>;
const IconCamera    = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 0 1 5.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 0 0-1.134-.175 2.31 2.31 0 0 1-1.64-1.055l-.822-1.316a2.192 2.192 0 0 0-1.736-1.039 48.774 48.774 0 0 0-5.232 0 2.192 2.192 0 0 0-1.736 1.039l-.821 1.316Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0ZM18.75 10.5h.008v.008h-.008V10.5Z" /></svg>;
const IconCheck     = ({ className = "w-3.5 h-3.5" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>;
const IconX         = ({ className = "w-3.5 h-3.5" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>;
const IconLock      = ({ className = "w-5 h-5" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>;
const IconLogout    = ({ className = "w-4 h-4" }) => <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9" /></svg>;
const IconSpinner   = ({ className = "w-4 h-4" }) => <svg className={`animate-spin ${className}`} fill="none" viewBox="0 0 24 24"><circle className="opacity-20" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>;

// ── Btn ───────────────────────────────────────────────────────────────────────

function Btn({ onClick, disabled, loading, variant = "ghost", icon, children, className = "" }: {
  ...
  const v = {
    ...
    ghost:   "bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200",
    danger:  "bg-red-500/10 hover:bg-red-500/20 border border-red-300 dark:border-red-500/20 text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300",
  }[variant];
  return (
...
function Toast({ msg, onClose }: { msg: string; onClose: () => void }) {
  return (
    <div className={`flex items-start gap-2 rounded-lg px-3 py-2.5 text-xs border ${bgSub} border-slate-200 dark:border-white/10 ${txtBody}`}>
      <span className="flex-1">{msg}</span>
      <button onClick={onClose} className={`shrink-0 ${txtDim} hover:${txtBody} cursor-pointer transition-colors`}><IconX /></button>
    </div>
  );
}
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="w-full max-w-xs">
          <div className="mb-8 text-center">
            <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-orange-500/10 border border-orange-200 dark:border-orange-500/20 mb-4">
              <IconLock className="w-5 h-5 text-orange-500 dark:text-orange-400" />
            </div>
            <h1 className={`text-lg font-semibold ${txtPri}`}>Pannello Admin</h1>
          </div>
          <form onSubmit={handleLogin} className={`${card} p-5 space-y-3`}>
            <div>
              <label htmlFor="ak" className={`block text-[11px] font-medium ${txtMut} mb-1.5 uppercase tracking-wider`}>
                Chiave admin
              </label>
              <input id="ak" type="password" value={keyInput} onChange={e => setKeyInput(e.target.value)}
                className={`w-full px-3 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-1 focus:ring-orange-500/60 transition-colors bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 ${txtPri} placeholder-slate-400 dark:placeholder-slate-600`}
    ...
    <div className="max-w-5xl space-y-4 pb-16">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <h1 className={`text-base font-semibold ${txtPri}`}>Dashboard</h1>
          {stats && (
            <span className={`flex items-center gap-1.5 text-[11px] ${txtMut}`}>
        ...
        <button onClick={handleLogout}
          className={`flex items-center gap-1.5 text-xs ${txtMut} hover:text-slate-700 dark:hover:text-slate-300 transition-colors cursor-pointer px-2 py-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className={`${card} p-5 flex flex-col justify-between min-h-[110px]`}>
          <p className={`text-[11px] font-medium uppercase tracking-widest ${txtMut}`}>Articoli totali</p>
          {stats
            ? <p className={`text-4xl font-bold ${txtPri} tabular-nums mt-2`}>{stats.total_articles}</p>
            : <div className={`h-10 w-20 mt-2 ${skel}`} />}
        </div>

        <div className={`${card} p-5 flex flex-col justify-between min-h-[110px]`}>
          <p className={`text-[11px] font-medium uppercase tracking-widest ${txtMut}`}>Ultime 24h</p>
          {stats ? (
            <>
              <p className="text-4xl font-bold text-emerald-600 dark:text-emerald-400 tabular-nums mt-2">{stats.articles_last_24h}</p>
              <div className={`mt-3 h-1 w-full rounded-full ${track} overflow-hidden`}>
          ...
          ) : <div className={`h-10 w-16 mt-2 ${skel}`} />}
        </div>

        <div className={`${card} p-5 flex flex-col justify-between min-h-[110px]`}>
          <p className={`text-[11px] font-medium uppercase tracking-widest ${txtMut}`}>RSS in coda</p>
          {stats ? (
            <>
              <p className="text-4xl font-bold text-sky-600 dark:text-sky-400 tabular-nums mt-2">{stats.rss_items_pending}</p>
              <p className={`text-[11px] ${txtDim} mt-1`}>{stats.rss_items_processed} processati</p>
            </>
          ) : <div className={`h-10 w-14 mt-2 ${skel}`} />}
        </div>

        <div className={`${card} p-5 flex flex-col justify-between min-h-[110px]`}>
          <p className={`text-[11px] font-medium uppercase tracking-widest ${txtMut}`}>Ultimo articolo</p>
          {stats?.last_article_at ? (
            <>
              <p className={`text-lg font-semibold ${txtSec} mt-2 tabular-nums leading-tight`}>
                {fmt(stats.last_article_at, { hour: "2-digit", minute: "2-digit" })}
              </p>
              <p className={`text-[11px] ${txtMut} mt-1`}>
                {fmt(stats.last_article_at, { day: "2-digit", month: "short" })}
              </p>
            </>
          ) : <div className={`h-8 w-20 mt-2 ${skel}`} />}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {[
          ...
          const barCls  = pct >= 60 ? "bg-emerald-500" : pct >= 30 ? "bg-amber-400" : "bg-red-500";
          const valCls  = pct >= 60 ? "text-emerald-600 dark:text-emerald-400" : pct >= 30 ? "text-amber-600 dark:text-amber-400" : "text-red-600 dark:text-red-400";
          return (
            <div key={label} className={`${card} px-5 py-4`}>
              <div className="flex items-center justify-between mb-2">
                <p className={`text-[11px] font-medium uppercase tracking-widest ${txtMut}`}>{label}</p>
                <span className={`text-xl font-bold tabular-nums ${valCls}`}>{pct}%</span>
              </div>
              ...
              <p className={`text-[11px] ${txtDim} mt-1.5`}>{count} su {total} articoli con più fonti</p>
            </div>
          );
        })}
      </div>
      ...
      <div className={`${card} p-5 flex flex-col gap-4`}>
        <div className="flex items-center justify-between">
          <div>
            <h2 className={`text-sm font-semibold ${txtPri}`}>Pipeline</h2>
            <p className={`text-[11px] ${txtMut} mt-0.5`}>Automatica ogni 30 min</p>
          </div>
      ...
      <div className={`${card} p-5`}>
        <div className="flex items-center justify-between mb-4">
          <h2 className={`text-sm font-semibold ${txtPri}`}>
            Fonti RSS <span className={`${txtMut} font-normal`}>({Object.keys(FEED_META).length})</span>
          </h2>
          <div className={`flex items-center gap-3 text-[10px] ${txtDim}`}>
            <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-slate-600 inline-block" />item</span>
            <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />multi-fonte</span>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
          {sortedFeeds.map(({ domain, meta, count, multi }) => (
            <div key={domain} className={`flex items-center gap-3 py-2 border-b ${border} last:border-0`}>
              <div className="w-14 shrink-0">
                <div className={`h-1 w-full rounded-full ${track} overflow-hidden`}>
                  <div className="h-full rounded-full bg-orange-500/50 transition-all duration-500"
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <span className={`text-[10px] font-mono ${txtDim}`}>{count || "·"}</span>
                {multi > 0 && (
                  <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
                    {multi}×
                  </span>
                )}
              </div>
      ...
      <div className={`${card} p-5 space-y-5`}>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className={`text-sm font-semibold ${txtPri}`}>Instagram</h2>
            <p className={`text-[11px] ${txtMut} mt-0.5`}>09:00 · 12:30 · 21:00</p>
          </div>
          <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full border ${
            igRunning
              ? "bg-pink-50 dark:bg-pink-500/10 border-pink-200 dark:border-pink-500/20 text-pink-600 dark:text-pink-400"
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          {[
            ...
            <div key={l} className={`rounded-lg ${bgSub} border border-slate-200 dark:border-white/5 py-3`}>
              ...
              <p className={`text-[10px] ${txtMut} mt-0.5`}>{l}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {igMessage && <Toast msg={igMessage} onClose={() => setIgMessage(null)} />}
        </div>

        {/* Divider */}
        <div className={`border-t ${border}`} />

        {/* Coda + Ultimi post side by side */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            {(igStats?.too_old.length ?? 0) > 0 && (
              <div>
                <p className="text-[11px] font-medium uppercase tracking-widest text-amber-600/80 dark:text-amber-600/60 mb-2">
                ...
                <div className={`divide-y ${divider}`}>
                  {igStats!.too_old.map(a => (
                    <div key={a.id} className="flex items-center justify-between py-2 gap-2">
                      <a href={`/article/${a.id}`} target="_blank" rel="noopener noreferrer"
                        className={`text-xs ${txtDim} line-through truncate flex-1 hover:text-amber-500/80 transition-colors cursor-pointer`}>
                        {a.title}
                      </a>
                      <span className={`text-[10px] font-mono ${txtDim} shrink-0`}>▲{a.relevance_score}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            ...
            {(igStats?.recent_posted.length ?? 0) === 0 ? (
              <p className={`text-xs ${txtDim} italic`}>Nessun post ancora pubblicato.</p>
            ) : (
              <div className={`divide-y ${divider}`}>
                {igStats!.recent_posted.map(a => (
                  <div key={a.id} className="flex items-center gap-3 py-2.5">
                    <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-emerald-100 dark:bg-emerald-500/15 border border-emerald-300 dark:border-emerald-500/25 shrink-0">
      ...
      {/* ── Storico pipeline ─────────────────────────────────────────────── */}
      <div className={`${card} p-5`}>
        <h2 className={`text-sm font-semibold ${txtPri} mb-3`}>
          Storico pipeline <span className={`${txtMut} font-normal`}>({pipelineHistory.length})</span>
        </h2>
        {pipelineHistory.length === 0 ? (
          <p className={`text-xs ${txtDim} italic`}>Nessuna esecuzione registrata.</p>
        ) : (
          <>
            <div className={`divide-y ${divider}`}>
              {pipelineHistory.map((run) => {
                ...
                return (
                  <div key={run.id} className="flex items-center gap-4 py-1.5 text-[11px] font-mono">
                    <span className={`w-28 shrink-0 ${txtMut}`}>
                      {fmt(run.started_at, { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                    </span>
                    {isRunning
                      ? <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400"><IconSpinner className="w-3 h-3" />in corso</span>
                      : <span className={`${txtDim} w-10 shrink-0`}>{run.duration_s}s</span>}
                    <span className="text-emerald-600 dark:text-emerald-500">+{run.created}</span>
                    <span className="text-sky-600 dark:text-sky-500">~{run.updated}</span>
                    <span className={txtDim}>/{run.skipped}</span>
                    {run.errors > 0 && <span className="text-red-500 dark:text-red-400">✗{run.errors}</span>}
                  </div>
                );
              })}
            </div>
            <div className={`flex items-center gap-4 mt-2 pt-2 border-t ${border} text-[10px] ${txtDim}`}>
              ...
              <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-slate-400 inline-block" />saltati</span>
      ...
      {/* ── Zona pericolosa ──────────────────────────────────────────────── */}
      <div className="rounded-xl border border-red-200 dark:border-red-500/15 bg-red-50 dark:bg-red-500/[0.02] p-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-red-600 dark:text-red-400">Zona pericolosa</h2>
            <p className={`text-xs ${txtMut} mt-0.5`}>Azione irreversibile.</p>
          </div>
      ...
      <p className={`text-[10px] ${txtDim} font-mono`}>{API_BASE}</p>
    </div>
  );
}
