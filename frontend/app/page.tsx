...
function FeaturedLargeCard({ article }: { article: ArticleSummary }) {
  ...
  return (
    <Link href={`/article/${article.id}`} className="card-blue block overflow-hidden group h-full">
      {article.image_url && (
        <div className="w-full h-52 overflow-hidden bg-blue-50">
      ...
      <div className="p-6">
        <div className="flex items-center gap-2 mb-3">
          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${THREAT_PILL[level]}`}>
            {THREAT_ICON[level]} {LEVEL_LABELS[level]}
          </span>
          <time className="text-xs text-gray-400">{formatDateShort(article.published_at)}</time>
        </div>
        <h3 className="font-bold text-[#0B1F3A] text-lg leading-snug line-clamp-2 mb-4 group-hover:text-blue-600 transition-colors">
          {article.title}
        </h3>
        {article.summary && (
          <div className="bg-blue-50 border-l-[3px] border-blue-500 rounded-r-xl p-3 mb-4">
            <p className="text-xs text-blue-700 font-semibold mb-1 flex items-center gap-1">🤖 Byte dice:</p>
            <p className="text-sm text-blue-600 italic leading-relaxed line-clamp-2">{article.summary}</p>
          </div>
        )}
...
function FeaturedSmallCard({ article }: { article: ArticleSummary }) {
  ...
  return (
    <Link
      ...
      className="card-blue flex items-start gap-3 p-4 group"
      style={{ borderRadius: "16px" }}
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-1.5">
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${THREAT_PILL[level]}`}>
            {THREAT_ICON[level]} {LEVEL_LABELS[level]}
          </span>
        </div>
        <h4 className="text-sm font-semibold text-[#0B1F3A] line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
          {article.title}
        </h4>
      ...
      {article.image_url && (
        <div className="shrink-0 w-16 h-14 rounded-xl overflow-hidden bg-blue-50">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={article.image_url}
            alt=""
...
function DailyBriefing({ articles }: { articles: ArticleSummary[] }) {
  ...
  return (
    <section className="relative bg-[#0B1F3A] rounded-3xl overflow-hidden my-14">
      ...
      <div className="relative z-10 px-8 py-10 md:flex items-center gap-14">
        <div className="flex-1 min-w-0">
          <span className="text-[#06E6D9] text-xs font-bold tracking-widest uppercase mb-2 block">
            ⚡ Byte&apos;s Daily Briefing
          </span>
          <h2 className="text-white font-extrabold text-2xl mb-6">Le minacce di oggi</h2>
          ...
          <Link
            href="/"
            className="mt-7 inline-flex items-center gap-2 px-5 py-2.5 bg-[#06E6D9] text-[#0B1F3A] rounded-full text-sm font-bold hover:bg-cyan-300 transition-colors"
          >
        ...
        <div className="shrink-0 mt-10 md:mt-0 flex justify-center">
          <div className="relative w-44 h-44 float-anim">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/byte-mascot.png"
              ...
              onError={(e) => {
                ...
                if (el.parentElement) {
                  el.parentElement.innerHTML =
                    `<div style="width:176px;height:176px;border-radius:50%;background:rgba(37,99,235,0.2);display:flex;align-items:center;justify-content:center;font-size:72px;">👻</div>`;
                }
              }}
            />
...
export default function HomePage() {
  ...
  useEffect(() => {
    getArticles({ min_score: 8, limit: 6 })
      .then((res) =>
        setInEvidenza(
          res.items.filter((a) => isRecent(a.published_at, EVIDENZA_HOURS)).slice(0, 4)
        )
      )
      .catch(() => {});
    ...
    <div className="fade-up">
      ...
      {inEvidenza.length > 0 && (
        <section className="mb-14">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-xl font-extrabold text-[#0B1F3A]">⚠ In Evidenza</h2>
            <span className="text-xs text-gray-400 border border-blue-100 rounded-full px-2.5 py-0.5 bg-blue-50">
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2 flex-wrap mb-4">
          <span className="text-xs text-gray-500 font-medium mr-1">Rilevanza:</span>
          {([0, 1, 2, 3] as const).map((lvl) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                levelFilter === lvl
                  ? "border-blue-600 bg-blue-600 text-white shadow-blue-sm"
                  : "border-blue-200 text-gray-600 hover:border-blue-400 hover:text-blue-600 bg-white"
              }`}
            >
        ...
        {topTags.length > 0 && (
          <div>
            <button
              ...
              className="flex items-center gap-2 text-xs font-medium text-gray-500 hover:text-blue-600 transition-colors px-3 py-1.5 border border-blue-100 hover:border-blue-300 rounded-lg bg-white shadow-blue-sm"
            >
              ...
              <div className="mt-10 flex items-center justify-center gap-2">
                {page > 1 && (
                  <button
                    ...
                    className="px-5 py-2 text-sm rounded-full border border-blue-200 text-[#0B1F3A] font-medium hover:border-blue-400 hover:bg-blue-50 transition-all shadow-blue-sm"
                  >
                    ← Precedente
                  </button>
                )}
                ...
                {page < totalPages && (
                  <button
                    onClick={() => setPage(page + 1)}
                    className="px-5 py-2 text-sm rounded-full bg-[#0B1F3A] text-white font-medium hover:bg-blue-700 transition-colors shadow-blue-md"
                  >
                    Successiva →
                  </button>
                )}
    ...
    <article className="card-blue group flex flex-col overflow-hidden">
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center justify-between gap-2 mb-3">
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${THREAT_PILL[level]}`}>
            {THREAT_ICON[level]} {LEVEL_LABELS[level]}
          </span>
        ...
        <Link
          href={`/article/${article.id}`}
          className="font-bold text-[#0B1F3A] leading-snug line-clamp-2 mb-3 hover:text-blue-600 transition-colors"
        >
          {article.title}
        </Link>
        {article.summary && (
          <div className="bg-blue-50 border-l-[3px] border-blue-500 rounded-r-lg p-3 mb-4">
            <p className="text-xs text-blue-700 font-semibold mb-1 flex items-center gap-1">🤖 Byte dice:</p>
            <p className="text-xs text-blue-600 italic leading-relaxed line-clamp-3">{article.summary}</p>
          </div>
        )}
        <div className="flex flex-wrap gap-1.5 mb-4">
        ...
        <div className="mt-auto flex items-center justify-between text-xs text-gray-400">
          ...
          <Link
            href={`/article/${article.id}`}
            className="text-blue-600 font-semibold hover:translate-x-1 transition-transform inline-block"
          >
            Leggi →
          </Link>
        </div>
