...
def get_pending_articles(engine, max_posts: int = 1, hours: int = 36) -> list[dict]:
    ...
    q = text("""
        SELECT a.id, a.title, a.summary, a.body, a.tags, a.relevance_score,
               a.ig_carousel_data, a.image_url,
        ...
        GROUP BY a.id, a.title, a.summary, a.body, a.tags, a.relevance_score,
                 a.ig_carousel_data, a.image_url, a.ig_score
        ORDER BY effective_ig_score DESC, a.published_at DESC
        LIMIT :limit
    """)
    with engine.connect() as conn:
        rows = conn.execute(q, {"cutoff": cutoff, "limit": limit}).fetchall()
...
def _run_pipeline_locked(max_posts: int = 1) -> dict:
    ...
    if not articles:
        logger.info("Nessun articolo pendente per Instagram nelle ultime 36h")
        return {"posted": 0}
